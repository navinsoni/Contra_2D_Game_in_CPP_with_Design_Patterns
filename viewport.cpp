...
void Viewport::update() {
  ...
  if(position[0]>viewPoint[0]){
  	viewPoint[0] = position[0];
  }
  //if (position[0] < gdata->getXmlInt("worldWidthStart")) position[0] = gdata->getXmlInt("worldWidthStart");
  if (position[0] < viewPoint[0]) { position[0] = viewPoint[0]; }
